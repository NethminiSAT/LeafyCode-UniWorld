...
const Users = ({users}) => {
  ...
  return (
    <div className="w-full flex flex-col justify-center">
      ...
      <div className="flex">
        {users.map((user) => (
          <div className="h-full bg-white py-6 justify-center " key={user._id}>
              ...
              <div className="bg-pink-50 shadow-lg border-gray-100 border sm:rounded-xl p-8 flex mr-10 ">
                ...
                <div className="flex flex-col  space-y-4 justify-center">
                  <div className="flex justify-between items-start">
                    <h2 key={user.name} className="text-2xl font-bold">{user.name}</h2>
                  </div>
                  <p key={user.email} className=" text-gray-400 max-h-40 overflow-y-hidden">{user.email}</p>
